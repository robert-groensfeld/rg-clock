define(["exports", "./css-parse.js", "./style-settings.js", "./style-transformer.js", "./style-util.js", "./style-properties.js", "./style-placeholder.js", "./style-info.js", "./style-cache.js", "./document-watcher.js", "./template-map.js", "./apply-shim-utils.js", "./common-utils.js", "./custom-style-interface.js"], function (_exports, _cssParse, _styleSettings, _styleTransformer, StyleUtil, _styleProperties, _stylePlaceholder, _styleInfo, _styleCache, _documentWatcher, _templateMap, ApplyShimUtils, _commonUtils, _customStyleInterface) {
  ...
  */
   ...
   * @const {StyleCache}
   */
  const styleCache = new _styleCache.default();
  ...
  class ScopingShim {
    constructor() {
      ...
      let ast = new _cssParse.StyleNode();
      ast['rules'] = [];
      this._documentOwnerStyleInfo = _styleInfo.default.set(this._documentOwner, new _styleInfo.default(ast));
    ...
    prepareTemplateStyles(template, elementName, typeExtension) {
      ...
      let hasMixins = !StyleUtil.elementHasBuiltCss(template) && (0, _commonUtils.detectMixin)(cssText);
      let ast = (0, _cssParse.parse)(cssText); // only run the applyshim transforms if there is a mixin involved
      ...
      if (!ownPropertyNames.length || _styleSettings.nativeCssVariables) {
        let root = _styleSettings.nativeShadow ? template.content : null;
        let placeholder = (0, _stylePlaceholder.getStylePlaceholder)(elementName);
    ...
    _generateStaticStyle(info, rules, shadowroot, placeholder, cssBuild) {
      let cssText = _styleTransformer.default.elementStyles(info, rules, null, cssBuild);
    ...
    _prepareHost(host) {
      ...
      let placeholder = (0, _stylePlaceholder.getStylePlaceholder)(is);
      ...
      const styleInfo = new _styleInfo.default(ast, placeholder, ownStylePropertyNames, is, typeExtension, cssBuild); // only set the style info after this element has registered its template

      if (template) {
        _styleInfo.default.set(host, styleInfo);
      }
    ...
    styleElement(host, overrideProps) {
      let styleInfo = _styleInfo.default.get(host);
    ...
    styleElementNativeVariables(host, styleInfo) {
      ...
      const template = _templateMap.default[is]; // bail early if there is no shadowroot for this element
    ...
    _styleOwnerForNode(node) {
      ...
      if (host) {
        if (_styleInfo.default.get(host)) {
          return host;
        } else {
    ...
    _applyStyleProperties(host, styleInfo) {
      ...
      let style = _styleProperties.default.applyElementStyle(host, styleInfo.styleProperties, styleInfo.scopeSelector, cachedStyle);
    ...
    _updateProperties(host, styleInfo) {
      ...
      let ownerStyleInfo = _styleInfo.default.get(owner);

      let ownerProperties = ownerStyleInfo.styleProperties;
      let props = Object.create(ownerProperties || null);

      let hostAndRootProps = _styleProperties.default.hostAndRootPropertiesForScope(host, styleInfo.styleRules, styleInfo.cssBuild);

      let propertyData = _styleProperties.default.propertyDataFromStyles(ownerStyleInfo.styleRules, host);
    ...
    getComputedStyleValue(element, property) {
      ...
      if (!_styleSettings.nativeCssVariables) {
        ...
        let styleInfo = _styleInfo.default.get(element) || _styleInfo.default.get(this._styleOwnerForNode(element));
    ...
    setElementClass(element, classString) {
      ...
      if (!_styleSettings.nativeCssVariables) {
        let styleInfo = _styleInfo.default.get(element);
    ...
    _styleInfoForNode(node) {
      return _styleInfo.default.get(node);
    }
    /**
     * @param {!Element} node
     * @param {string} scope
